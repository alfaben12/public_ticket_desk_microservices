...
module.exports = {
	...
	processSetupMyaccount: function(req, res) {
		...
		let check = Recharge.MemberDetail
			.findAll({
				where: {
					member_id: member_id
				}
			...
			.then(function(rows) {
				if (rows.length == 1) {
					...
					Recharge.MemberDetail.create(value).then(function() {
						res.json({
							result: true,
	...
	processSetupCC: function(req, res) {
		...
		Recharge.PaymentCard.create(value).then(function(result) {
			res.json({
				result: true,
	...
	processDeleteCC: function(req, res) {
		...
		let check = Recharge.PaymentCard
			.findAll({
				where: {
					id: id,
					member_id: member_id
